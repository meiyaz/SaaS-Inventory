                ...
                <div className="lg:hidden divide-y divide-slate-100">
                    {bomItems.length === 0 ? (
                        <div className="px-5 py-10 text-center text-slate-400">
                            ...
                            <p className="text-sm">No items added yet. Use the form below to add products.</p>
                        </div>
                    ) : (
                        bomItems.map((item) => (
                            <div key={item.id} className="p-4 hover:bg-slate-50">
                                <div className="flex justify-between items-start gap-2 mb-2">
                                    <div className="min-w-0 pr-4">
                                        <div className="font-bold text-sm text-slate-900 truncate">{item.products?.name}</div>
                                        <div className="text-[10px] text-slate-400 font-mono mt-0.5">{item.products?.sku}</div>
                                    </div>
                                    <button onClick={() => handleDeleteLine(item.id)} className="p-1.5 text-slate-400 hover:text-red-500 transition-colors shrink-0 bg-white border border-slate-200 rounded shadow-sm">
                                        <Trash2 className="w-4 h-4" />
                                    </button>
                                </div>
                                <div className="flex flex-wrap items-center justify-between gap-2 mt-3 bg-slate-50 rounded p-2 text-sm border border-slate-100">
                                    <div className="flex gap-4">
                                        <div><span className="text-[10px] text-slate-400 uppercase tracking-wider block leading-none mb-1">Qty</span><span className="font-semibold text-slate-800">{item.quantity}</span></div>
                                        <div><span className="text-[10px] text-slate-400 uppercase tracking-wider block leading-none mb-1">Unit</span><span className="text-slate-700">₹{Number(item.unit_sell_price).toFixed(2)}</span></div>
                                    </div>
                                    <div className="text-right">
                                        <span className="text-[10px] text-slate-400 uppercase tracking-wider block leading-none mb-1">Total</span>
                                        <span className="font-bold text-slate-900">₹{(item.quantity * item.unit_sell_price).toFixed(2)}</span>
                                    </div>
                                </div>
                                {item.notes && (
                                    <div className="mt-2 text-xs text-slate-600 italic bg-amber-50/50 rounded px-2 py-1.5 border-l-2 border-amber-300">
                                        {item.notes}
                                    </div>
                                )}
                            </div>
                    ...
                    <form onSubmit={handleAddLine} className="flex flex-wrap gap-3 items-end">
                        <div className="flex-1 min-w-[200px]">
                            ...
                            <Select value={selectedProduct} onChange={handleProductSelect}
                                ...
                                options={products.map(p => ({ value: p.id, label: `[${p.current_stock} in stock] ${p.sku} – ${p.name}` }))}
                            />
                        </div>
                        <div className="w-24">
